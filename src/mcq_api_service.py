    ...
    **Authentication**: Include X-API-Key header if API_KEY is configured
    ...
    try:
        ...
        response = MCQResponse(
            ...
            message=f"Successfully generated {len(mcq_sets)} MCQ set with 25 questions for {category}"
        )
        
        logger.info(f"Successfully generated {len(mcq_sets)} set(s) with 25 questions for {category}")
        return response
